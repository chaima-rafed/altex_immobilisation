...
class AssetInventory1(models.Model):
    _name = 'asset.inventory.verify2'
    _rec_name = 'departement_id'
    _description = 'Asset Inventory Verify'
    _inherit = ['mail.thread']


    get_inventory_verify_line = fields.One2many('asset.inventory.verify.line2', 'asset_inventory_verify',
    ...
    # Delete all selected row of 'asset.inventory.verify2' and his get_inventory_verify_line rows or records
    def unlink(self):
        for record in self:
    ...
    def get_assets(self):
        ...
        if self.code_bare:
            domain = [
                ...
                ('company_name', '=', self.env.company.id),
                '|',
                ('departement_id', '=', self.departement_id.id),
                ('desktop_id', '=', self.desktop_id.id)
            ]
