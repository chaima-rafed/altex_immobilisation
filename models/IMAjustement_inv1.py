...
class IMAdjustement_inv1(models.Model):
    _name = 'im.adjustement.inv1'
    _description = "Adjustement d'inventaire1"

    im_adjustement1 = fields.One2many('immo.inv', 'im_adjustement_many1', string="IM Adjustement 01")
    ...
    def refresh_database(self):
        ...
        get_assets_inv_verify = self.env['asset.inventory.verify1'].search([])
        ...
        for asset in get_assets_inv_verify:
            ...
            for line in asset.get_inventory_verify_line:
                ...
                existing_adjustement = self.env['im.adjustement.inv1'].search([
                ...
                if existing_adjustement:
                    ...
                    # Create new record
                    self.env['im.adjustement.inv1'].create(data)
    ...
    def refresh_databaseAll(self):
        ...
        all_inventory_verifications = self.env['asset.inventory.verify1'].search([])
        ...
        for asset in all_assets:
            ...
            if inventory_verification:
                inventory_line = inventory_verification.mapped('get_inventory_verify_line1').filtered(
                ...
                if inventory_line:
                    ...
                    existing_adjustement = self.env['im.adjustement.inv1'].search([('asset_id', '=', asset.id)])
    ...
    def compute_and_display_differences(self, values):
        ...
        differences = self.env['asset.inventory.difference1'].compute_d(self.get_selected_records(differences))
        if differences:
            print(f'deferrence true')
            # Open a new window with the differences
            action = self.env.ref('altex_imobilisation.action_asset_inventory_difference1').read()[0]
            ...
            menu_action = self.env.ref('altex_imobilisation.action_asset_inventory_difference1').read()[0]
    ...
    def get_selected_records(self, differences):
        selected_records = self.env['im.adjustement.inv1'].browse(self._context.get('active_ids', []))
