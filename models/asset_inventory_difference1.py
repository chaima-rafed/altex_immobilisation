...
class AssetInventoryDifference1(models.Model):
    _name = 'asset.inventory.difference1'
    _description = 'Asset Inventory Difference1'
    ...
    def compute_differences(self):
        ...
        asset_inventory_verify_records = self.env['im.adjustement.inv1'].search([])

        for inventory_verify_record in asset_inventory_verify_records:
            # search if exist on asset.inventory.difference1 or not
            get_asset_inventorydifference = self.env['asset.inventory.difference1'].search([
    ...
    def compute_d(self, differences):
        ...
        if differences:
            for inventory_verify_record in differences:
                ...
                # search if exist on asset.inventory.difference1 or not (dublicated)
                get_asset_inventorydifference = self.env['asset.inventory.difference1'].search([
                ...
                if not get_asset_inventorydifference:
                    ...
                    #todo Create records in asset.inventory.difference1
                    self.create(diff)


    # Update asset_verify field of asset.inventory.verify1 model to False after delete it
    def update_related_verify_records(self):
        ...
        # Find related records in asset.inventory.verify1 and update asset_verify to False
        verify_records = self.env['asset.inventory.verify1'].search([('asset_id', 'in', asset_ids)])
        if verify_records:
    ...
    def unlink(self):
        print('1')
        # Update related records in asset.inventory.verify1 before deleting
        # self.update_related_verify_records()
        print('6')
    
        # Call the unlink method of the superclass
        return super(AssetInventoryDifference1, self).unlink()
    ...
    def write(self, vals):
        ...
        if self.asset_id:
            ...
            return super(AssetInventoryDifference1, self).write(vals)
